...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 adapt=True, down_size=64, adapt_scalar="0.5", init_value="0.5", layernorm_option="in",
                 patch_wise_scalar=True, fusion_method='concat'):
        super().__init__()
    ...
    def forward(self, x, return_attention=False):
        ...
        if self.adapt:
            # print(f'adapt has been executed')
            # print(f'patch_wise_scalar : {self.patch_wise_scalar}')
            if self.patch_wise_scalar:
                if self.fusion_method == 'concat':
                    scalar = self.scalar_pred(torch.cat([origin_mlp_x, adapt_x], dim=-1))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm,adapt=True ,**kwargs):
        super().__init__()
